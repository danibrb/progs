...
def create_domain_window(df):
  ...
  domain_label.pack()
  global domain_entry
  domain_entry = Entry(domain_window, insertwidth=0, textvariable=StringVar(value="calvino.edu.it"))
  ...
  export_button = Button(domain_window, text="Export Data (CSV)", command=lambda: export_data(df.copy()))
  export_button.pack()
